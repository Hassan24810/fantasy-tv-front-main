-- Drop and recreate get_user_leagues_with_stats with new return structure
DROP FUNCTION IF EXISTS public.get_user_leagues_with_stats(uuid);
...
BEGIN
  ...
  -- Get leagues with stats including aggregated points
  ...
  LEFT JOIN LATERAL (
    SELECT 
      ...
      COALESCE(SUM(su.total_points), 0)::bigint as total_league_points,
      COALESCE(SUM(su.gw_points), 0)::bigint as total_league_gw_points
    FROM public.league_members lm2
    INNER JOIN public.show_users su ON su.id = lm2.show_user_id
...
-- Drop and recreate get_leagues_with_stats with new return structure
DROP FUNCTION IF EXISTS public.get_leagues_with_stats(uuid);
...
DECLARE
  v_result jsonb;
BEGIN
  -- Get all leagues for this show with stats
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'id', l.id,
      'name', l.name,
  ...
  LEFT JOIN LATERAL (
    ...
    INNER JOIN public.show_users su ON su.id = lm.show_user_id
