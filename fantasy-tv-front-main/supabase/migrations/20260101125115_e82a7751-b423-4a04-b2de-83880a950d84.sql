-- RPC: get_user_points_breakdown
-- Returns summary points and detailed event breakdown for the authenticated user
-- Uses EXACT same visibility/reveal logic as calculate_user_total_points and calculate_user_gameweek_points
...
BEGIN
  ...
  event_with_user_points AS (
    SELECT 
      ...
      -- Sum points only for participants on user's team for this episode
      COALESCE(SUM(
        CASE 
      ...
      -- Contributing participants (only those on user's team)
      jsonb_agg(
        CASE 
          WHEN utm.participant_id IS NOT NULL 
               AND utm.added_episode <= re.episode_number 
      ...
      -- All event participants (for full template display)
      jsonb_agg(
        jsonb_build_object(
          'participant_id', ep.participant_id,
          'participant_name', p.name,
