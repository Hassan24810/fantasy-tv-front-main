...
interface EditUpdateSheetProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  update: ShowUpdate | null;
}

export function EditUpdateSheet({ open, onOpenChange, update }: EditUpdateSheetProps) {
  ...
  useEffect(() => {
    if (update) {
      form.reset({
        ...
        publish_at: format(new Date(update.publish_at), "yyyy-MM-dd'T'HH:mm"),
        expire_at: update.expire_at
          ? format(new Date(update.expire_at), "yyyy-MM-dd'T'HH:mm")
          : "",
        is_enabled: update.is_enabled ?? true,
        media_url: (update as any).media_url || "",
      });
      setMediaPreview((update as any).media_url || null);
    }
  }, [update, form]);
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxSize) {
      toast.error(t("updates.fileTooLarge", { max: isVideo ? "50MB" : "10MB" }));
      return;
    }
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-[500px] overflow-y-auto">
        <SheetHeader>
          <SheetTitle>{t("updates.editUpdate")}</SheetTitle>
          <SheetDescription>{t("updates.editUpdateDescription")}</SheetDescription>
        </SheetHeader>
